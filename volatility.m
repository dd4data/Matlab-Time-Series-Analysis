...
snp=readmatrix('S&P 500.xlsx'); ftse=readmatrix('FTSE.xlsx'); 
dax=readmatrix('DAX.xlsx'); cac=readmatrix('CAC.xlsx'); 
mib=readmatrix('MIB.xlsx'); nikkei=readmatrix('Nikkei.xlsx'); 
hk=readmatrix('Hang Seng.xlsx');  sh=readmatrix('Shanghai.xlsx'); 

% compute volatility:
% for each table, 1st column:date; 2nd:closed; 3rd:open; 4th:high;5th:low.
vsnp=zeros(size(snp,1),1);
for ii=1:1:size(snp,1)
    vsnp(ii,:)=0.511*(log(snp(ii,4))-log(snp(ii,5)))^2-0.019*((log(snp(ii,2))...
        -log(snp(ii,3)))*(log(snp(ii,4))+log(snp(ii,5))-2*log(snp(ii,3)))...
        -2*(log(snp(ii,4))-log(snp(ii,3)))*(log(snp(ii,5))-log(snp(ii,3))))...
        -0.383*(log(snp(ii,2))-log(snp(ii,3)))^2;
end

vftse=zeros(size(ftse,1),1);
for ii=1:1:size(ftse,1)
    vftse(ii,:)=0.511*(log(ftse(ii,4))-log(ftse(ii,5)))^2-0.019*((log(ftse(ii,2))...
        -log(ftse(ii,3)))*(log(ftse(ii,4))+log(ftse(ii,5))-2*log(ftse(ii,3)))...
        -2*(log(ftse(ii,4))-log(ftse(ii,3)))*(log(ftse(ii,5))-log(ftse(ii,3))))...
        -0.383*(log(ftse(ii,2))-log(ftse(ii,3)))^2;
end

vdax=zeros(size(dax,1),1);
for ii=1:1:size(dax,1)
    vdax(ii,:)=0.511*(log(dax(ii,4))-log(dax(ii,5)))^2-0.019*((log(dax(ii,2))...
        -log(dax(ii,3)))*(log(dax(ii,4))+log(dax(ii,5))-2*log(dax(ii,3)))...
        -2*(log(dax(ii,4))-log(dax(ii,3)))*(log(dax(ii,5))-log(dax(ii,3))))...
        -0.383*(log(dax(ii,2))-log(dax(ii,3)))^2;
end

vcac=zeros(size(cac,1),1);
for ii=1:1:size(cac,1)
    vcac(ii,:)=0.511*(log(cac(ii,4))-log(cac(ii,5)))^2-0.019*((log(cac(ii,2))...
        -log(cac(ii,3)))*(log(cac(ii,4))+log(cac(ii,5))-2*log(cac(ii,3)))...
        -2*(log(cac(ii,4))-log(cac(ii,3)))*(log(cac(ii,5))-log(cac(ii,3))))...
        -0.383*(log(cac(ii,2))-log(cac(ii,3)))^2;
end

vmib=zeros(size(mib,1),1);
for ii=1:1:size(mib,1)
    vmib(ii,:)=0.511*(log(mib(ii,4))-log(mib(ii,5)))^2-0.019*((log(mib(ii,2))...
        -log(mib(ii,3)))*(log(mib(ii,4))+log(mib(ii,5))-2*log(mib(ii,3)))...
        -2*(log(mib(ii,4))-log(mib(ii,3)))*(log(mib(ii,5))-log(mib(ii,3))))...
        -0.383*(log(mib(ii,2))-log(mib(ii,3)))^2;
end

vnik=zeros(size(nikkei,1),1);
for ii=1:1:size(nikkei,1)
    vnik(ii,:)=0.511*(log(nikkei(ii,4))-log(nikkei(ii,5)))^2-0.019*((log(nikkei(ii,2))...
        -log(nikkei(ii,3)))*(log(nikkei(ii,4))+log(nikkei(ii,5))-2*log(nikkei(ii,3)))...
        -2*(log(nikkei(ii,4))-log(nikkei(ii,3)))*(log(nikkei(ii,5))-log(nikkei(ii,3))))...
        -0.383*(log(nikkei(ii,2))-log(nikkei(ii,3)))^2;
end

vhk=zeros(size(hk,1),1);
for ii=1:1:size(hk,1)
    vhk(ii,:)=0.511*(log(hk(ii,4))-log(hk(ii,5)))^2-0.019*((log(hk(ii,2))...
        -log(hk(ii,3)))*(log(hk(ii,4))+log(hk(ii,5))-2*log(hk(ii,3)))...
        -2*(log(hk(ii,4))-log(hk(ii,3)))*(log(hk(ii,5))-log(hk(ii,3))))...
        -0.383*(log(hk(ii,2))-log(hk(ii,3)))^2;
end

vsh=zeros(size(sh,1),1);
for ii=1:1:size(sh,1)
    vsh(ii,:)=0.511*(log(sh(ii,4))-log(sh(ii,5)))^2-0.019*((log(sh(ii,2))...
        -log(sh(ii,3)))*(log(sh(ii,4))+log(sh(ii,5))-2*log(sh(ii,3)))...
        -2*(log(sh(ii,4))-log(sh(ii,3)))*(log(sh(ii,5))-log(sh(ii,3))))...
        -0.383*(log(sh(ii,2))-log(sh(ii,3)))^2;
end

% to change the date form:
...
vSNP=[snpdates, vsnp];  % change the date into matlab datenum

ftsedates=datetime(ftse(:,1),'ConvertFrom','excel');
ftsedates=datenum(ftsedates); 
vFTSE=[ftsedates, vftse];
...
vNIKKEI =[nidates, vnik];

hsdates=datetime(hk(:,1),'ConvertFrom','excel');
...
vola=zeros(size(bdates,1), 9); % to take trading days data following the USA

for i=1:size(bdates,1)
    vola(i,1)=bdates(i,1);
    
for j=1:size(vSNP,1)
if  vola(i,1)==vSNP(j,1)
       vola(i,2)=vSNP(j,2); 
end
end
end
...
for i=1:size(bdates,1)
for k=1:size(vFTSE,1)
if  vola(i,1)==vFTSE(k,1)
       vola(i,3)=vFTSE(k,2);
...
for i=1:size(bdates,1)
for o=1:size(vNIKKEI,1)
if  vola(i,1)==vNIKKEI(o,1)
       vola(i,7)=vNIKKEI(o,2);
...
vola=vola(3:end,2:end); % stock market volatilities are ready, take those from Jan 4th, 2006 to May 31th, 2019 as returns!!!
                           % holidays followed from USA.
for i=1:size(vola,1)
    for j=1:size(vola,2)
    if vola(i,j)==0
        vola(i,j)=NaN;  % to find missing data.
    end
        
    end
end

vola=fillmissing(vola,'previous',1); % fill missing data with previous ones.
y=vola'; % transform observations into KxT dimension.
...
diff=abs(OC_out-OC_in)./OC_in; % compute the relative difference of country-wise Out- and In- relative outgoing connectedness
...
event=datenum('13/02/09',formatIn); % enter the date of an event here!!!
for dd=1:1:size(Datestr,1)
    if datenum(Datestr(dd,:),formatIn)==event
       Z=dd; %to find out the point of the date
    end
end
special=Datestr(2477,:); % enter data point in the first argument!!!

%% relative difference of outgoing connectedness plot
